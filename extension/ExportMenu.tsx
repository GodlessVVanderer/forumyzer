...
export default function ExportMenu({ forumId, threads }: ExportMenuProps) {
  ...
  const createShareLink = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backendUrl}/api/forum/${forumId}/share`, {
        method: 'POST',
        headers: {
      ...
      const data = await res.json();
      setShareUrl(`${backendUrl}/api/forum/share/${data.shareToken}`);
    } catch (err: any) {
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => exportForumAsJSON(forumId, threads)}
        style={buttonStyle}
      >
        📋 Export JSON
      </button>
      {/* PDF export could be added here */}
      <button onClick={createShareLink} style={buttonStyle} disabled={loading}>
